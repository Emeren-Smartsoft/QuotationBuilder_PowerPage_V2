...
## Dell Quote Pricelist (2 flows)

The Quotation page has a fourth mode-tab, **"Dell Quote"**, that treats each Dell quotation email as a small reusable pricelist. A Power Automate flow ingests Dell quote emails into Excel files under SharePoint `Documents/Pricelist/Dell/<QuoteNo>.xlsx` (built from `Documents/Templates/Dell-template.xlsx`). A second HTTP flow serves the browser: it both lists available Dell quote files and returns the SKU rows of a specific file. The contract intentionally mirrors **Flow 5 (QuoteSite-GetWorkstationData)** so the client code can reuse the same renderer.

### Flow 6: QuoteSite-IngestDellPricelist

Outlook-triggered background flow. Whenever a Dell quote email arrives, AI Builder extracts the quote header + line items, then the flow copies `Templates/Dell-template.xlsx` to `Pricelist/Dell/<QuoteNo> - <CustomerName>.xlsx` and appends one row per line item into the new file's `Table1`.

| Request | Behaviour |
|---------|-----------|
| (Outlook email arrives matching the trigger filter) | A new Excel file appears under `Documents/Pricelist/Dell/` named `<QuoteNo> - <CustomerName>.xlsx`, populated with the line items. No HTTP response — runs in background. |

### SharePoint setup (one-time)

1. Open the **Documents** library in the SmartBox site.
2. Create folder `Pricelist/Dell/` (if it doesn't already exist).
3. Create folder `Templates/`.
4. Build the empty template workbook:
   - Open Excel → new blank workbook.
   - Type headers in row 1: `SKU` | `Description` | `Qty` | `UnitPrice`.
   - Select the header row → **Insert → Table** → tick "My table has headers" → OK.
   - Table Design tab → **Table Name** = `Table1`.
   - Save As → upload to SharePoint as `Documents/Templates/Dell-template.xlsx`.

---

### Action 1: Trigger – "When a new email arrives (V3)" (Office 365 Outlook)

Search: `When a new email arrives (V3)`

> Sign in with the mailbox that receives Dell quotes (or a shared mailbox configured for it).

| Field | Value |
|-------|-------|
| Folder | `Inbox` (or `Inbox/Dell Quotes` if you create a server-side rule that moves Dell mail into a subfolder — recommended) |
| **Advanced parameters → Show all:** | |
| From | `*@dell.com` (add partner domains separated by `;` if needed) |
| Subject Filter | `Quote` (Dell quote subjects always contain the word) |
| Include Attachments | `Yes` |
| Only with Attachments | `Yes` (skips text-only chase mails) |
| Importance | `Any` |

---

### Action 2: Build the AI Builder prompt (one-time, outside the flow)

> Recent AI Builder tenants no longer expose the old **"Create text with GPT using a prompt"** action. Instead you build a **saved prompt** in the AI Builder prompt designer and then call it from the flow with **"Run a prompt"** (visible in your AI Builder action list — bottom-left of the screenshot).
>
> ⚠️ The prompt designer was rebuilt in 2025 and no longer has a separate "Inputs" side panel. Inputs are added **inline** by typing `/` inside the Instructions box (the placeholder text confirms this: *"Use '/' to add inputs and knowledge"*).

Do this **once**, before adding Action 3 below:

1. Open <https://make.powerautomate.com> → left nav **AI hub** → **Prompts** → click the **"Build your own prompt"** card. (Or from inside the flow, click **+ New step → AI Builder → Run a prompt → Create new prompt**.)
2. **Name** the prompt at the top of the page: `Dell Quote Extractor`.
3. **Model** dropdown (top of the Instructions card, e.g. shows `GPT-4.1 mini ▾`): pick **GPT-4o** if available — better at strict JSON. GPT-4.1 mini also works.
4. **Output** dropdown (top-right of the right panel): leave as **Text**. Do **not** switch to JSON mode (it requires a schema and the flow's Parse JSON handles it fine).
5. Click inside the **Instructions** box and paste the first part of the prompt:
   ```
   Extract the Dell quote header and line items from the email body below. Return STRICT JSON only (no markdown, no commentary, no code fences) with this exact shape:

   {"quoteNo":"string","customerName":"string","quoteDate":"YYYY-MM-DD","salesRep":"string","lines":[{"sku":"string","description":"string","qty":0,"unitPrice":0}]}

   --- EMAIL BODY ---
   ```
6. **Press Enter** to start a new line, then **type `/`** (forward slash). A small popup appears with options (**Text input**, **Knowledge**, **Image**…).
7. Click **Text input** → in the dialog set **Name** = `EmailBody` → click **Add**. A coloured **EmailBody** chip is inserted at the cursor.
   - *Fallback:* if `/` does nothing, click the **+ Add content** button under the Instructions box → **Input** → **Text** → same dialog.
8. **Test it**: click **Test** at the top-right of the Instructions card → paste a sample Dell quote email body into the `EmailBody` test input → **Test**. The **Model response** panel on the right should show clean JSON.
9. Click **Save custom prompt** (bottom-right of the page — scroll/expand if it's cut off).

---

### Action 3: "Run a prompt" (AI Builder)

Search: `Run a prompt` (AI Builder connector — this is the tile shown bottom-left in your screenshot).

| Field | How to fill |
|-------|-------------|
| Prompt | Click → pick **`Dell Quote Extractor`** (the prompt you saved in Action 2). |
| EmailBody | After picking the prompt, an input box named **EmailBody** appears. Click → **Dynamic content** tab → from the **When a new email arrives (V3)** trigger pick **Body** (the HTML/text body field). Or **Expression** tab → `triggerOutputs()?['body/body']` → **Add**. |

> Requires an AI Builder licence (or AI Builder credits attached to the environment). If neither is available, replace this action with **HTTP** → POST to Azure OpenAI / OpenAI; everything downstream stays the same as long as the JSON string ends up in `body('<action>')?['…']?['text']`.
...
| Field | How to fill |
|-------|-------------|
| Content | **Expression** tab → `body('Run_a_prompt')?['responsev2']?['predictionOutput']?['text']` → **Add** |
| Schema | Paste the JSON below |

> ⚠️ The exact output path depends on the AI Builder action version. If `body('Run_a_prompt')?['responsev2']?['predictionOutput']?['text']` returns `null` in a test run, open the run history → expand the **Run a prompt** step → look at the raw **Outputs** JSON and pick the field that holds the generated text (common alternatives: `body('Run_a_prompt')?['text']` or `outputs('Run_a_prompt')?['body']?['text']`).
...
### Action 5: "Compose" – sanitise the customer name

Search: `Compose`

- **Rename** the action to `SafeCustomer`.
- **Inputs** – **Expression** tab → paste:
  ```
  trim(replace(replace(replace(replace(replace(replace(replace(replace(replace(coalesce(body('Parse_JSON')?['customerName'], 'Unknown Customer'), '\', ' '), '/', ' '), ':', ' '), '*', ' '), '?', ' '), '"', ' '), '<', ' '), '>', ' '), '|', ' '))
  ```
  → **Add**.

> Strips the 9 characters SharePoint disallows in filenames (`\ / : * ? " < > |`) and falls back to `Unknown Customer` if the AI didn't extract a name.

---

### Action 6: "Compose" – build the new filename

Search: `Compose`

- **Rename** the action to `NewFileName`.
- **Inputs** – **Expression** tab → paste:
  ```
  concat(body('Parse_JSON')?['quoteNo'], ' - ', outputs('SafeCustomer'), '.xlsx')
  ```
  → **Add**.

> Result example: `3700023958518.3 - Smartsoft Business Solutions.xlsx`.

---

### Action 7: "Get file content using path" (SharePoint)
...
|-------|-------|
| Site Address | `Smartsoft - https://z2bm1.sharepoint.com/sites/SmartBox` |
| File Path | `/Shared Documents/Templates/Dell-template.xlsx` |

---

### Action 8: "Create file" (SharePoint)
...
|-------|-------------|
| Site Address | `Smartsoft - https://z2bm1.sharepoint.com/sites/SmartBox` |
| Folder Path | `/Shared Documents/Pricelist/Dell` |
| File Name | Click → **Dynamic content** → select **Outputs** from `NewFileName`. (Or **Expression** → `outputs('NewFileName')` → **Add**.) |
| File Content | **Dynamic content** → **File Content** from `Get file content using path`. (Or **Expression** → `body('Get_file_content_using_path')` → **Add**.) |

> ⚠️ Power Automate writes the file synchronously, but the Excel Online connector sometimes can't see brand-new files for ~5 seconds. If Action 10 below intermittently fails with "File not found", add a **Delay** action (5 seconds) between Action 8 and Action 9.

---

### Action 9: "Apply to each" – over the parsed line items

Search: `Apply to each`

| Field | How to fill |
|-------|-------------|
| Select An Output From Previous Steps | **Dynamic content** → select **lines** from `Parse JSON`. (Or **Expression** → `body('Parse_JSON')?['lines']` → **Add**.) |

**Action 9a (inside the loop): "Add a row into a table"** (Excel Online (Business))

Search: `Add a row into a table`

| Field | How to fill |
|-------|-------------|
| Location | `Smartsoft - https://z2bm1.sharepoint.com/sites/SmartBox` (the SharePoint site, NOT OneDrive) |
| Document Library | `Documents` |
| File | Click in field → **Enter custom value** → **Expression** tab → `concat('/Pricelist/Dell/', outputs('NewFileName'))` → **Add** |
| Table | Click in field → **Enter custom value** → type `Table1` (plain text) |
| Row → SKU | **Expression** → `items('Apply_to_each')?['sku']` → **Add** |
| Row → Description | **Expression** → `items('Apply_to_each')?['description']` → **Add** |
| Row → Qty | **Expression** → `items('Apply_to_each')?['qty']` → **Add** |
| Row → UnitPrice | **Expression** → `items('Apply_to_each')?['unitPrice']` → **Add** |

> The `Row` section appears once you pick the table. If Power Automate can't resolve the table dynamically and the Row fields don't appear, run the flow once with a known good file, then re-open the action — it will populate the columns.

---

### Action 10 (optional): "Update file properties" (SharePoint)

If you want the SharePoint Documents library to expose `CustomerName`, `QuoteDate`, `SalesRep` columns for search and filtering, add **Update file properties** here and map those library columns to the corresponding `body('Parse_JSON')?['customerName']` / `quoteDate` / `salesRep` expressions. Otherwise skip this step.
...
1. Click **Save**.
2. From your inbox, forward (or send) a sample Dell quote email matching the trigger filters.
3. Open Power Automate → My flows → `QuoteSite-IngestDellPricelist` → **28-day run history** → click the latest run.
   - Each step should show green ✓.
   - Expand `Parse JSON` to verify the AI extracted `quoteNo`, `customerName`, and `lines[]`.
   - Open SharePoint → `Documents/Pricelist/Dell/` — the new file should be there with all line items in `Table1`.

> **Duplicate-skip tip:** before Action 8, add **Get file metadata using path** (path = `concat('/Shared Documents/Pricelist/Dell/', outputs('NewFileName'))`) with **Settings → Configure run after → also "has failed"** ticked, followed by a Condition: if it returned 200 (i.e. the file already exists), drop a **Terminate** with status `Succeeded`. Otherwise continue.

---

### Flow 7: QuoteSite-GetDellPricelist

HTTP-triggered. ONE flow handles two cases based on the request body — mirrors **Flow 5** exactly so the same browser-side renderer applies.

| Request body | Behaviour |
|--------------|-----------|
| `{}` | Returns the list of Dell-quote filenames (every `.xlsx`, sans extension, under `Documents/Pricelist/Dell/`). |
| `{ "list": "3700023958518.3 - Smartsoft Business Solutions" }` | Reads `Documents/Pricelist/Dell/<that name>.xlsx` → `Table1`, returns each row as a SKU object. |
...
> Save the flow once so the **HTTP POST URL** is generated — copy that URL; you'll paste it into `flowUrls.getDellPricelist` later.
...
- **Rename** the action to `ListName`.
- **Inputs** – **Expression** tab → paste:
  ```
  coalesce(triggerBody()?['list'], '')
  ```
  → **Add**.

> Yields an empty string when the browser sends `{}`.
...
| Field | How to fill |
|-------|-------------|
| Left side | **Expression** tab → `length(outputs('ListName'))` → **Add** |
| Operator | `is equal to` |
| Right side | Type: `0` |

---

### If yes → return the list of Dell-quote filenames
...
| Site Address | `Smartsoft - https://z2bm1.sharepoint.com/sites/SmartBox` |
| Method | `GET` |
| Uri | `_api/web/GetFolderByServerRelativeUrl('/sites/SmartBox/Shared Documents/Pricelist/Dell')/Files?$select=Name` |
| Headers – Key | `Accept` |
| Headers – Value | `application/json;odata=nometadata` |

**Action 3b: "Select"** (Data Operations)

| Field | How to fill |
|-------|-------------|
| From | **Expression** tab → `body('Send_an_HTTP_request_to_SharePoint')?['value']` → **Add** |
| Map | Click the **T** toggle (top-right of Map) to switch to text mode → **Expression** tab → `replace(item()?['Name'], '.xlsx', '')` → **Add** |
...
| Headers – Key 2 | `Access-Control-Allow-Origin` |
| Headers – Value 2 | `*` |
| Body | Type plain text: `{"lists": @{body('Select')}}` |

---

### If no → return SKU rows of the chosen file

**Action 3d: "List rows present in a table"** (Excel Online (Business))

| Field | How to fill |
|-------|-------------|
| Location | `Smartsoft - https://z2bm1.sharepoint.com/sites/SmartBox` (the SharePoint site — NOT OneDrive) |
| Document Library | `Documents` |
| File | Click → **Enter custom value** → **Expression** tab → `concat('/Pricelist/Dell/', outputs('ListName'), '.xlsx')` → **Add** |
| Table | Click → **Enter custom value** → type `Table1` (plain text) |

**Action 3e: "Select"** (Data Operations) — keep only the four useful columns (drops `__PowerAppsId__` automatically).

- **From** (expression): `body('List_rows_present_in_a_table')?['value']`
- **Map** (leave in default key/value mode, add four rows):

  | Key | Value (Expression tab) |
  |-----|------------------------|
  | `SKU` | `item()?['SKU']` |
...
| Headers – Key 2 | `Access-Control-Allow-Origin` |
| Headers – Value 2 | `*` |
| Body | Type plain text: `{"skus": @{body('Select')}}` |

> ⚠️ If your inner Select gets renamed `Select_1` (because the outer Select in the True branch already used `Select`), update the body to `{"skus": @{body('Select_1')}}` to match the actual action name shown in the designer.
...
1. Click **Save**.
2. Go back to the trigger step → copy the **HTTP URL**.
3. Paste it into `productData.flowUrls.getDellPricelist` in `Quotation.en-US.webpage.copy.html`.
4. Run:
   ```
   pac pages upload --path "D:\VS_CopilotStudio_Agents\Quote Site\quotation---site-j0yx8" --modelVersion 2
   ```
...
```js
// List Dell quote files
fetch(URL, {method:"POST",headers:{"Content-Type":"application/json"},body:"{}"})
  .then(r => r.json()).then(console.log);
// Expected: {"lists": ["3700023958518.3 - Smartsoft Business Solutions", ...]}

// SKUs from one quote
fetch(URL, {method:"POST",headers:{"Content-Type":"application/json"},
  body: JSON.stringify({list:"3700023958518.3 - Smartsoft Business Solutions"})})
  .then(r => r.json()).then(console.log);
// Expected: {"skus": [{"SKU":"210-BFRI","Description":"Dell OptiPlex 7010 SFF…","Qty":2,"UnitPrice":62500}, ...]}
```
### Browser config key

In the Quotation page's `<script id="productData">` block, add one URL alongside the existing entries:

```json
{
  "flowUrls": {
    "getDellPricelist": "<Flow 7 invoke URL>"
  }
}
```

If the URL is missing or still starts with `PASTE_`, the Dell tab shows a configuration-required message — there is no fallback fixture in this build.
...
├── Pricelist/
│   ├── Dell/
│   │   ├── 3700023958518.3 - Smartsoft Business Solutions.xlsx   ← <QuoteNo> - <CustomerName>
│   │   └── 3700024080237.2 - Acme Pvt Ltd.xlsx
│   ├── Softwares/                       (Flow 2)
│   └── ...
├── MOQ Prices/                          (Flow 5)
└── Templates/
    └── Dell-template.xlsx               ← empty workbook with named Table1 (SKU,Description,Qty,UnitPrice)
```
